...
- O 정익절: 단기 스윙 수익실현 / 장중 10분 간격 모니터링 / 5~8종목 (전일 대비 총자산 +5% 전 종목 익절, -3% 전 종목 손절)
...
- `scripts/cron/o_monitor_cron.sh` → `scripts/core/o_monitor.py`
  - 10분 간격 총자산 체크, 전일 대비 +5% 전 종목 익절 / -3% 전 종목 손절
  - 장마감(15:20) 자동 종료
...
- O (단기 스윙 수익실현): 모멘텀+기술적 분석으로 단기 반등 종목 선별, 5~8종목. **신규 진입 종목만 allocation에 포함**. 전일 대비 총자산 +5% 달성 시 전 종목 익절, -3% 시 전 종목 손절은 장중 o_monitor.py(실시간) 또는 simulate.py(과거 날짜)가 자동 처리
