...
- `scripts/cron/q_monitor_cron.sh` → `scripts/core/q_monitor.py`
  - **09:30 ~ 14:00 진입 윈도우**, HOLDING은 최대 14:30까지 모니터링 (14:00 마지막 진입 + 30분)
  - 시그널 발견 즉시 시장가 매수 (당일 재매수 금지 가드)
  - 매수 후 현재가 체크 30초 간격 / IDLE 스캔 1분 간격 → **+2.5% 익절 / -1.5% 손절 / 30분 시간청산** (트레일링·post5_vol 모두 폐기)
  - 동시 보유 **1종목** (HOLDING 중에는 신규 스캔 스킵)
  - **종목 선정 v2.1 — 풀 199종목 1분봉 직접 평가**
    - 풀: KOSPI200 ∪ stock_universe = 199종목 (`build_pool()` 매 실행 시 결합, KIS 거래대금 상위 KOSDAQ 보강은 OOS 검증에서 독으로 확인되어 미적용)
    - 매분 풀 전 종목 1분봉 6개 (`FHKST03010230`) 동시 호출 (ThreadPool 10)
