...
- Q 정채원: 장중 1분 상시 스캔 스캘핑 / 09:00~14:50 1분 간격 거래량 폭증 종목 발견 즉시 매수 → 매수+30분 안에 청산 / 1종목 집중 (직전 15분 거래량 ≥3배(2배 fallback) + 등락률 ≥+5% + 전일 종가 ≥ 2,000원, 9시대는 당일 누적 거래량 1위 fallback, +4% 익절/-3% 손절/매수+30분 강제 청산, 동시 보유 1종목, 당일 재매수 금지, 매매당 max 1,000만원 캡, **stock_universe 무관, 전체 KOSPI/KOSDAQ 대상 매매**)
...
# Q 정채원 1분 상시 스캔 모니터링 (09:00~14:50, 거래량 ≥3배+등락률 ≥+5% 발견 즉시 매수 → 매수+30분 청산)
...
### 오전 8:45 — Q 정채원 1분 상시 스캔 스캘핑 (2026-04-29~ 거래량 폭증 매집 추종)
- plist: `~/Library/LaunchAgents/com.investment.q-monitor.plist`
- `scripts/cron/q_monitor_cron.sh` → `scripts/core/q_monitor.py`
  - 09:00 ~ 14:50 동안 **1분 간격 KIS 등락률 순위 상시 스캔**
  - 거래량 폭증 종목 발견 즉시 시장가 매수 (당일 재매수 금지 가드)
  - 매수 후 30분 모니터링 (1분 간격 가격 체크) → +4% 익절 / -3% 손절 / 매수+30분 강제 청산
  - 동시 보유 **1종목** (HOLDING 중에는 신규 스캔 스킵)
  - 종목 선정 (4단계): ①등락률 ≥ +5% (KIS `FHPST01700000`) → ②전일 종가 ≥ 2,000원 → ③9시대(09:00~09:59)는 분봉 비교 불가, 당일 누적 거래량 1위 fallback → ④10시 이후는 직전 15분 거래량 / 전일 동시간대 (`FHKST03010230`) ≥ **3배** (3배 후보 없으면 ≥ **2배**로 완화)
...
scripts/
  core/              # 시뮬레이션 핵심 엔진
    ...
    q_monitor.py         Q 정채원 1분 상시 스캔 스캘핑 (09:00~14:50 거래량 ≥3배+등락률 ≥+5% 발견 즉시 매수 → 매수+30분 +4%/-3% 청산, KIS 등락률+직전 15분 분봉 거래량, universe 외부 종목)
...
- Q 정채원: 빠르고 단호한 스캘퍼, 결단력 있는 톤 ("거래량 터지자마자 잡았다", "+4% 찍고 바로 던졌다", "오늘은 폭증 후보 없어서 관망", "30분 안에 결판낸다")
