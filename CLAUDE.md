...
- Q 정채원: 장중 1분 상시 스캔 스캘핑 / 10:00~14:50 1분 간격 거래량 폭증 종목 발견 즉시 매수 → 매수+30분 안에 청산 / 1종목 집중 (직전 15분 거래량 ≥4배(3배 fallback) + 등락률 ≥+7% + 전일 종가 ≥ 2,000원, 9시대 진입 차단, **트레일링 익절(+3% 활성→고점 -1%p 되돌림, v3 5→3)** / -3% 손절 / 매수+30분 강제 청산, 동시 보유 1종목, 당일 재매수 금지, 일일 매매 한도 8회, 직전 3사이클 연속 손실 시 60분 쿨다운, **bear 레짐 신규 진입 차단 / bull_score≤2 시 임계 ×2 자동 상향 (등락률 +14%, 거래량 8배/6배, v3)**, 매매당 max 1,000만원 캡, **stock_universe 무관, 전체 KOSPI/KOSDAQ 대상 매매**)
...
# Q 정채원 1분 상시 스캔 모니터링 v3 (10:00~14:50, 거래량 ≥4배+등락률 ≥+7% 발견 즉시 매수 → 매수+30분 청산, bear 진입 차단)
...
### 오전 8:45 — Q 정채원 1분 상시 스캔 스캘핑 v3 (2026-05-06~ 5/4·5/6 백테스트 반영)
- plist: `~/Library/LaunchAgents/com.investment.q-monitor.plist`
- `scripts/cron/q_monitor_cron.sh` → `scripts/core/q_monitor.py`
  - **10:00 ~ 14:50 동안 1분 간격 KIS 등락률 순위 상시 스캔** (9시대 진입 차단 — 분봉 비교 불가 + 04-30 0승 2패 데이터)
  - 거래량 폭증 종목 발견 즉시 시장가 매수 (당일 재매수 금지 가드)
  - 매수 후 30분 모니터링 (HOLDING 30초 간격, IDLE 스캔 1분 간격) → **트레일링 익절(+3% 활성→고점 -1%p 되돌림, v3 5→3)** / -3% 손절 / 매수+30분 강제 청산
  - 동시 보유 **1종목** (HOLDING 중에는 신규 스캔 스킵)
  - 종목 선정 (3단계): ①등락률 ≥ **+7%** (KIS `FHPST01700000`) → ②전일 종가 ≥ 2,000원 → ③직전 15분 거래량 / 전일 동시간대 (`FHKST03010230`) ≥ **4배** (4배 후보 없으면 ≥ **3배**로 완화)
  - **레짐 게이트 (v3 강화)**: 시작 시 `market_regimes` 조회 →
      · **bear 레짐 → 신규 진입 차단** (HOLDING 중이면 청산 후 종료, 5/6 약세 휩쏘 회피)
      · **bull_score ≤ 2 (neutral/bull) → 임계 ×2 자동 상향** (등락률 +14%, 거래량 8배/6배)
      · 정상 (score ≥ 3) → 기본 임계
...
scripts/
  core/              # 시뮬레이션 핵심 엔진
    ...
    q_monitor.py         Q 정채원 1분 상시 스캔 스캘핑 v3 (10:00~14:50 거래량 ≥4배+등락률 ≥+7% 발견 즉시 매수 → 매수+30분 트레일링(+3%활성→-1%p)/-3% 청산, KIS 등락률+직전 15분 분봉 거래량, 일일 8회 한도/3연패 60분 쿨다운, bear 진입 차단 / score≤2 임계 ×2, universe 외부 종목)
...
- **Q (1분 상시 스캔 스캘핑 v3): allocation 결정 단계 제외**. Claude 에이전트 만들지 않음 (Q는 q_monitor.py가 KIS API 등락률 순위·분봉 거래량 비교를 통해 1분 간격 직접 매매). simulate.py도 Q allocation이 None이면 자동 스킵.
...
- Q 정채원: 빠르고 단호한 스캘퍼, 결단력 있는 톤 ("거래량 터지자마자 잡았다", "트레일링 걸리고 -1% 이탈에서 던졌다", "오늘은 임계 7% 못 넘어서 관망", "30분 안에 결판낸다", "3연패 떠서 1시간 쉰다")
