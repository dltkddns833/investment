...
- Q 정채원: 장중 1분 상시 스캔 스캘핑 v5 / 10:00~10:30 1분 간격 **KOSPI200 대형주 + 1분봉 vol 시그널** 종목 발견 즉시 매수 → **post5_vol 동적 보유**(60/30/15분) 후 청산 / 1종목 집중 (KOSPI200 정적 리스트 198개 AND 1분봉 vol/5MA ≥3배 AND 현재 등락률 ≤+15% AND 5MA vol ≥1,000주 AND 전일 종가 ≥2,000원, 진입 10:30 마감, **트레일링 익절(+3% 활성→고점 -1%p 되돌림)** / -3% 손절 / 매수+5분 시점 post5_vol 측정해 동적 강제청산(vol≥1.0→60분, 0.5~1.0→30분, <0.5→15분), 동시 보유 1종목, 당일 재매수 금지, 일일 매매 한도 8회, 직전 3사이클 연속 손실 시 60분 쿨다운, **bear 레짐 신규 진입 차단**, 매매당 max 1,000만원 캡, **KOSPI200 정적 리스트만 매매 (kospi200.py)**, v4의 점프 감지/1m≥+2% 게이트는 87건 분석에서 표본 0건으로 폐기)
...
# Q 정채원 1분 상시 스캔 모니터링 v5 (10:00~10:30 진입, KOSPI200 대형주 + 1분봉 vol/5MA ≥3배 발견 즉시 매수 → post5_vol 동적 보유 60/30/15분, bear 진입 차단)
python3 scripts/core/q_monitor.py              # 실행 (08:45 시작, 10:00 스캔 개시)
...
### 오전 8:45 — Q 정채원 1분 상시 스캔 스캘핑 v5 (2026-05-13~ 87건 표본 분석 반영, 시간/대형주 게이트 도입)
- plist: `~/Library/LaunchAgents/com.investment.q-monitor.plist`
- `scripts/cron/q_monitor_cron.sh` → `scripts/core/q_monitor.py`
  - **10:00 ~ 10:30 진입 윈도우**, HOLDING은 최대 11:30까지 모니터링
    · 분석 D: 10:30 이후 대형주 진입은 음수 평균 (11:00~ -0.3%, 12:00~ -0.5%)
  - vol 시그널 발견 즉시 시장가 매수 (당일 재매수 금지 가드)
  - 매수 후 가격 체크 30초 간격 / IDLE 스캔 1분 간격 → **트레일링 익절(+3% 활성→고점 -1%p 되돌림)** / -3% 손절 / **post5_vol 동적 강제 청산**
  - 동시 보유 **1종목** (HOLDING 중에는 신규 스캔 스킵)
  - **종목 선정 v5 — KOSPI200 대형주 + 1분봉 vol 시그널 (v4 점프/1m≥+2% 게이트 폐기)**
    - 1차 narrow: 매분 KIS 등락률 순위 (`FHPST01700000`) 호출 → `kospi200.py`의 정적 198개 코드와 교집합 (분석 C: 대형주 +0.94% vs 중소형주 -0.70%)
    - 가드: 현재 등락률 ≤ +15% (고속 락 종목 회피), 전일 종가 ≥ 2,000원
    - 2차 confirm: 후보 종목만 1분봉 6개 (`FHKST03010230`) → **직전 1분 vol / 직전 5분 평균 ≥ 3배**
    - 휴면 차단: 직전 5분 평균 거래량 < 1,000주는 무시
    - API 호출량: 정상 1건(등락률 순위) + KOSPI200 양전 대형주만 1분봉 (보통 0~10건)
  - **post5_vol 동적 보유** (매수 +5분 시점에 1회 측정):
    · post5_vol_ratio = (진입 후 1~5분 평균 거래량) / (진입 시 5MA)
    · ≥1.0 (vol 유지/증가) → 매수+60분 강제청산 (단계적 상승 패턴)
    · 0.5~1.0 (보통)       → 매수+30분
    · <0.5 (vol 급감)      → 매수+15분 (시그널 소멸, 빠르게)
    · 측정 실패 시 NORMAL(30분) fallback
  - **레짐 게이트**: 시작 시 `market_regimes` 조회 →
      · **bear 레짐 → 신규 진입 차단** (HOLDING 중이면 청산 후 종료)
      · 그 외 → vol 시그널 그대로 적용
  ...
  - 종목 범위: **KOSPI200 정적 리스트 198개** (`scripts/core/kospi200.py`, 6개월마다 수동 갱신 권장)
  - 재기동 시 transactions에서 당일 거래 종목 + buy_count + 사이클 손익 자동 로드 → 재매수/한도/쿨다운 복원 (재인계 holding은 NORMAL 30분 fallback)
...
scripts/
  core/              # 시뮬레이션 핵심 엔진
    ...
    q_monitor.py         Q 정채원 1분 상시 스캔 스캘핑 v5 (10:00~10:30 진입, KOSPI200 대형주 + 1분봉 vol/5MA ≥3배 발견 즉시 매수 → post5_vol 동적 보유(60/30/15분) + 트레일링(+3%활성→-1%p)/-3% 청산, 등락률 ≤+15%/5MA vol ≥1,000주 가드, 일일 8회 한도/3연패 60분 쿨다운, bear 진입 차단, kospi200.py 정적 리스트)
...
- **Q (1분 상시 스캔 스캘핑 v5): allocation 결정 단계 제외**. Claude 에이전트 만들지 않음 (Q는 q_monitor.py가 KOSPI200 대형주 + 1분봉 vol 시그널로 10:00~10:30 진입 윈도우만 직접 매매, post5_vol 동적 보유). simulate.py도 Q allocation이 None이면 자동 스킵.
...
- Q 정채원: 빠르고 단호한 스캘퍼, 결단력 있는 톤 ("10시 첫 분에 KOSPI200 대형주에서 vol 4배 떠서 잡았다", "post5 1.2배라 60분 길게 갔다", "post5 0.3배로 시그널 꺼져서 15분에 던졌다", "트레일링 걸리고 -1% 이탈에서 던졌다", "오늘은 10시30분까지 vol 시그널 없어서 관망", "3연패 떠서 1시간 쉰다")
