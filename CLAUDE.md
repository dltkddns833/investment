...
scripts/
  core/              # 시뮬레이션 핵심 엔진
    ...
    q_monitor.py         Q 정채원 급락 반등 스캘핑 v2.1 (이슈 #60, 09:30~14:00 진입, 풀 199 1분봉 직접 평가 → 직전 5분 ≤ -2.5% + 양봉 ≥ +0.3% 시그널 → +2.5/-1.5/30m 단순 청산, 일일 8회 한도/3연패 60분 쿨다운, bear 진입 차단, 분 경계 +20초 정렬·near-miss 로깅·실패 종목 우선 재시도·universe code 6자리 정규화·ε=1e-3 보정)
