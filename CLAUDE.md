...
- Q 정채원: 장중 1분 상시 스캔 스캘핑 / 10:00~14:50 1분 간격 거래량 폭증 종목 발견 즉시 매수 → 매수+30분 안에 청산 / 1종목 집중 (직전 15분 거래량 ≥4배(3배 fallback) + 등락률 ≥+7% + 전일 종가 ≥ 2,000원, 9시대 진입 차단, 트레일링 익절(+5% 활성→고점 -1%p 되돌림)/-3% 손절/매수+30분 강제 청산, 동시 보유 1종목, 당일 재매수 금지, 일일 매매 한도 8회, 직전 3사이클 연속 손실 시 60분 쿨다운, 약세 레짐(bear or bull_score≤2) 시 모든 임계 ×1.5 자동 상향, 매매당 max 1,000만원 캡, **stock_universe 무관, 전체 KOSPI/KOSDAQ 대상 매매**)
...
# Q 정채원 1분 상시 스캔 모니터링 v2 (10:00~14:50, 거래량 ≥4배+등락률 ≥+7% 발견 즉시 매수 → 매수+30분 청산)
...
### 오전 8:45 — Q 정채원 1분 상시 스캔 스캘핑 v2 (2026-04-30~ 04-30 운영 데이터 반영)
...
- `scripts/cron/q_monitor_cron.sh` → `scripts/core/q_monitor.py`
  ...
  - 매수 후 30분 모니터링 (HOLDING 30초 간격, IDLE 스캔 1분 간격) → **트레일링 익절(+5% 활성→고점 -1%p 되돌림)** / -3% 손절 / 매수+30분 강제 청산
  ...
  - **레짐 게이트**: 시작 시 `market_regimes` 조회 → bear 또는 bull_score ≤ 2이면 모든 임계 ×1.5 자동 상향 (등락률 +10.5%, 거래량 6배/4.5배)
...
scripts/
  core/              # 시뮬레이션 핵심 엔진
    ...
    q_monitor.py         Q 정채원 1분 상시 스캔 스캘핑 v2 (10:00~14:50 거래량 ≥4배+등락률 ≥+7% 발견 즉시 매수 → 매수+30분 트레일링(+5%활성→-1%p)/-3% 청산, KIS 등락률+직전 15분 분봉 거래량, 일일 8회 한도/3연패 60분 쿨다운/약세 레짐 ×1.5, universe 외부 종목)
...
- **Q (1분 상시 스캔 스캘핑 v2): allocation 결정 단계 제외**. Claude 에이전트 만들지 않음 (Q는 q_monitor.py가 KIS API 등락률 순위·분봉 거래량 비교를 통해 1분 간격 직접 매매). simulate.py도 Q allocation이 None이면 자동 스킵.
