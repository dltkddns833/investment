...
17명의 가상 투자자(A~Q)가 서로 다른 **투자 성향**과 **리밸런싱 빈도**로 한국 주식 및 ETF에 투자하여 성과를 비교하는 시뮬레이션. A~P는 전용 데이터 모듈과 AI 에이전트를 통해 독립적으로 배분을 결정하고, Q 정채원은 KIS API 등락률 순위·분봉 거래량 비교로 매시간 직접 매매한다 (universe 무관, 전체 KOSPI/KOSDAQ).
...
| Q | 정채원 | 장중 7세션 스캘핑 | 공격투자형 | 매시간 정각 | 1 | 전일 종가 +10~15% 급등 + 직전 1시간 거래량 폭증 1개, +5% 익절/-3% 손절/XX:10 강제 청산, max 1,000만원 캡, **universe 외부 종목** |
...
2. 투자자별 독립 분석/배분 결정 (16개 AI 에이전트 병렬, A~P)
   ...
   → Q 정채원은 q_monitor.py가 별도 시간(08:45~)에 KIS API로 직접 매매
...
│   │   └── q_monitor.py                Q 정채원 7세션 스캘핑 (08:50 ATS → 정시 매수 → XX:10 청산)
...
| 시간 | 작업 | 실행 방식 |
|------|------|----------|
| **08:45** | Q 정채원 7세션 스캘핑 (08:50 ATS → 09/10/11/12/13/14/15시 매수 → XX:10 청산) | Python |
