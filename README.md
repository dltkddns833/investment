...
16명의 가상 투자자(A~P)가 서로 다른 **투자 성향**과 **리밸런싱 빈도**로 한국 주식 및 ETF에 투자하여 성과를 비교하는 시뮬레이션. 각 투자자는 전용 데이터 모듈과 AI 에이전트를 통해 독립적으로 배분을 결정한다.
...
[장마감 후 — 스토리텔링]
4. 코멘터리 & 투자자 일기 (16명 + 메타 매니저) 생성 → daily_stories 테이블
...
- 로그: `logs/{pipeline,o_monitor,p_monitor,meta,storytelling}/` 종류별 폴더 + `launchd/` (launchd stdout/stderr)
...
- **투자자 목록** (`/investors`): 전체 16명 카드 그리드, 순위/수익률/투자성향 뱃지
