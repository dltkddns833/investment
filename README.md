...
17명의 가상 투자자(A~Q)가 서로 다른 **투자 성향**과 **리밸런싱 빈도**로 한국 주식 및 ETF에 투자하여 성과를 비교하는 시뮬레이션. A~P는 전용 데이터 모듈과 AI 에이전트를 통해 독립적으로 배분을 결정하고, Q 정채원은 KIS API 등락률 순위·직전 15분 분봉 거래량 비교로 09:00~14:50 동안 1분 단위 상시 스캔하여 거래량 폭증(직전 15분 ≥3배, 2배 fallback) + 등락률 ≥+5% 종목을 발견 즉시 직접 매매한다 (universe 무관, 전체 KOSPI/KOSDAQ).
...
| Q | 정채원 | 장중 1분 상시 스캔 스캘핑 | 공격투자형 | 09:00~14:50 1분 간격 | 1 | 거래량 폭증(직전 15분 ≥3배, 2배 fallback) + 등락률 ≥+5% 발견 즉시 매수 → 매수+30분 청산, +4% 익절/-3% 손절, 9시대 누적거래량 1위 fallback, 동시 보유 1종목, 당일 재매수 금지, max 1,000만원 캡, **universe 외부 종목** |
...
[장마감 후 — 스토리텔링]
4. 코멘터리 & 투자자 일기 (17명 + 메타 매니저) 생성 → daily_stories 테이블
