...
17명의 가상 투자자(A~Q)가 서로 다른 **투자 성향**과 **리밸런싱 빈도**로 한국 주식 및 ETF에 투자하여 성과를 비교하는 시뮬레이션. A~P는 전용 데이터 모듈과 AI 에이전트를 통해 독립적으로 배분을 결정하고, Q 정채원은 KIS API 등락률 순위·분봉 거래량 비교로 09:00~15:10 동안 1분 단위 상시 스캔하여 +10~15% 밴드 발견 즉시 직접 매매한다 (universe 무관, 전체 KOSPI/KOSDAQ).
...
| Q | 정채원 | 장중 1분 상시 스캔 스캘핑 | 공격투자형 | 09:00~15:10 1분 간격 | 1 | +10~15% 밴드 발견 즉시 매수 → 매수+10분 청산, +5% 익절/-3% 손절, 동시 보유 1종목, 당일 재매수 금지, max 1,000만원 캡, **universe 외부 종목** |
...
2. 투자자별 독립 분석/배분 결정 (16개 AI 에이전트 병렬, A~P)
   ...
   → Q 정채원은 q_monitor.py가 별도 시간(08:45~)에 09:00~15:10 1분 간격 상시 스캔하여 KIS API로 직접 매매
...
│   │   └── q_monitor.py                Q 정채원 1분 상시 스캔 스캘핑 (09:00~15:10, +10~15% 발견 즉시 매수 → 매수+10분 청산)
...
| 시간 | 작업 | 실행 방식 |
|------|------|----------|
| **08:45** | Q 정채원 1분 상시 스캔 스캘핑 (09:00~15:10, +10~15% 발견 즉시 매수 → 매수+10분 청산) | Python |
