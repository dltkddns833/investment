---
name: simulate
description: "한국 주식 모의투자 시뮬레이션 파이프라인 실행. 뉴스 수집 → 16명 투자자 배분 결정(A~P 병렬) → 시가 체결 시뮬레이션. Q 정채원은 별도 q_monitor.py가 직접 매매하므로 Part A 대상이 아님. 사용자가 '시뮬레이션', '시뮬레이션 진행', '시뮬 돌려', 'Part A', '오늘 시뮬' 등을 언급하면 반드시 이 스킬을 사용한다."
---

# 시뮬레이션 파이프라인 (Part A)

한국 주식 모의투자 시뮬레이션 Part A는 **A~P 16명**의 배분 결정과 시가 체결을 담당한다. Q 정채원은 universe 무관 7세션 스캘핑이라 q_monitor.py가 별도 시각에 직접 매매하므로 Part A 대상이 아니다.
...
### Step 2: 투자자별 배분 결정 (16개 병렬 에이전트, A~P)

`notify("🧠 Step 2: 투자자별 배분 결정 시작 (16명 병렬)")`

**반드시 16개 서브에이전트(Agent tool)를 동시에 병렬 실행**한다. 각 에이전트는 독립적으로 자기 투자자의 배분만 결정한다. **Q 정채원은 에이전트를 만들지 않는다** (q_monitor.py가 직접 매매).
